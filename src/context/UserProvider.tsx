    ...
    const initializeTelegramWebApp = async () => {
      try {
        ...
        if (window.Telegram && window.Telegram.WebApp) {
          ...
          const userRef = doc(firestoreDB, "users", String(teleUser.id));
          const docSnap = await getDoc(userRef);

          let user : DocumentData | undefined = docSnap.data();

          let ball = user?.balance;
          ...
          setTimeout(() => {
            ...
            if (telegramId) {
              localStorage.setItem("chatId", String(telegramId));
