...
//   const [, setShowConfetti] = useRecoilState(confettiAtom)
  ...
  useEffect(() => {
    ...
    if (savedLastPumpTime) {
      ...
      let lastTime = new Date(savedLastPumpTime);
      const now = new Date();
  ...
  const [showConfettti, setShowConfettti] = useState(false);
  ...
  const handlePump = () => {
    ...
    setShowConfettti(true);
    setTimeout(() => {
      setShowConfettti(false);
    }, 3000);
  ...
  return (
    <DrawerContent className="flex flex-col items-center pt-6 pb-3">
      {showConfettti && (
        <Confetti numberOfPieces={1500} recycle={false} gravity={0.09} style={{zIndex:'100'}}/>
      )}
          ...
          <Button
            ...
            className={`font-extrabold text-[12px] leading-[18px] flex flex-col h-auto  ${
              collected[index]
                ...
                : (collected[index-1] || index == 0) && isPumpAvailable ? "bg-[#BAB5C9] border-2 border-transparent hover:bg-[#f7eded3d]": "bg-[#C3C3C33D] border-2 border-transparent hover:bg-[#f7eded3d]"
            }`}
          >
            <div>Day {index + 1}</div>
